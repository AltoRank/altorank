> **History, not the merge record.** This file was written on `integration/outrank-port`
> (PR #92), a parallel integration of the same wave that was superseded by
> `integration/wave-2026-09-04` (PR #91). #91's merge commits are the authoritative
> resolution notes for what lands; this is kept because it documents, per conflicted
> file, a second independent resolution of the same ~70 pairwise conflicts, and the two
> agree everywhere they overlap. Where they differed, #91's choice stands (its Rule-7
> unifications go further: one `PLAN_MAX_ENTRIES`, `lib/cms/delivery.ts`, `recordPublish`
> as the one `publish_log` writer, migration 062). The only things carried over from #92
> into #91 were the re-runnable `049/051/052/054/055` migrations and this file.
