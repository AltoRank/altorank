...
The wider fresh-business check subsequently exposed a real-size HTTP 414 failure in topic-group persistence and an automatic-drafting fallthrough after empty/failed explicit-choice planning. Those are corrected in `27f7bc0`; **migration 093 is also required before deployment**. The smaller 13 September fixtures did not expose the URL-size failure. See [the continued release evaluation](./onboarding-release-2026-09-14.md) for retained failures, the fixed 37 KB database regression, current production-schema inspection and the eight-business evaluation. This follow-up supersedes any unqualified readiness interpretation of the earlier mechanical checks.
