...
- **CLI** `npm run cli -- <group> <command>` from `apps/web` of an AltoRank
  checkout (or `tsx apps/web/scripts/cli.ts`). Same calls, prints the same
  envelope, exits 1 on failure.

The MCP server (`apps/web/scripts/mcp.ts`) exposes the public readiness tools
with no key, and the account tools below when `ALTORANK_API_KEY` is set.

`commands/` next to this file holds five thin slash commands (whoami,
readiness, keywords, draft, review-queue) that wrap the CLI calls below. They
are Claude Code command files; copy them into `.claude/commands/` to use them
as `/altorank-…`. They add nothing the CLI cannot do.
