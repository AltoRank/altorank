...
/**
 * `previous` is null when the comparison window was never synced (#75):
 * connecting Search Console backfills about a week, so for the first month a
 * dashed line drawn from that window would be a flat zero claiming last month
 * had no traffic. No line is the honest drawing.
 */
function Series({ label, current, previous, gradientId }: { label: string; current: number[]; previous: number[] | null; gradientId: string }) {
  ...
  const prev = previous ? pathFor(previous, W, H, pad, max) : null;
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="96" preserveAspectRatio="none" role="img" aria-label={`${label}, daily, current window against the previous one`}>
        ...
        {prev && <path d={prev.line} fill="none" stroke="var(--ink-4)" strokeWidth="1.5" strokeDasharray="4 4" />}
    ...
    <GscGate connected={connected} hasData={perf.hasData}>
      {!perf.hasClicks && perf.impressions.current === 0 && (perf.impressions.previous ?? 0) === 0 ? (
        ...
        <div className="grid gap-3">
          <Series label="Clicks" current={perf.current.map((p) => p.clicks)} previous={perf.previousMeasured ? perf.previous.map((p) => p.clicks) : null} gradientId="gsc-clicks" />
          <Series label="Impressions" current={perf.current.map((p) => p.impressions)} previous={perf.previousMeasured ? perf.previous.map((p) => p.impressions) : null} gradientId="gsc-impressions" />
          {!perf.hasClicks && (
