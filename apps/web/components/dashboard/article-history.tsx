            ...
            {shown.map((r) => (
              ...
              >
                ...
                <td className="px-3.5 py-3 border-b border-line-soft" onClick={(e) => e.stopPropagation()}>
                  <ArticleRowMenu articleId={r.id} currentStatus={r.status} canPublish={r.canPublish} publishTarget={r.publishTarget} canRetry={r.canRetry} held={r.held} autoApprove={Boolean(r.autoApproveAfter)} />
                </td>
              </tr>
            ))}
            {rows.length === 0 && (
