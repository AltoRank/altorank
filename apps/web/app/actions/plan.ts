...
import { cadenceDays, countScheduled, describePlanDiff, ensureQuestionsFor, previewPlan, schedulePlan, PLAN_MAX_ENTRIES, type PlanDiff } from "@/lib/onboarding/plan";
...
export async function planMonth(): Promise<{ planned: number; scheduled: number; max: number }> {
  ...
  const planned = await schedulePlan(supabase, workspaceId, pace, {
    from: new Date(),
    mode: "top-up",
