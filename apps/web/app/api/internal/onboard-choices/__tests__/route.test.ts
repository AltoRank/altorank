...
const mocks=vi.hoisted(()=>({rpc:vi.fn(),prepare:vi.fn(),deferred:[] as Array<()=>Promise<void>>}));
...
it("returns 202 after a durable claim and holds source work in the invocation",async()=>{
  ...
  expect(mocks.prepare).toHaveBeenCalledWith({rpc:mocks.rpc},runId,"lease-token");
});
