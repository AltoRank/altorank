...
test("a new account is walked from /dashboard to a planned first month", async ({ page, account }) => {
  ...
  const { data: account } = await db.from("accounts").select("attribution_source, attribution_answered_at").eq("id", account.accountId).single();
  expect(account?.attribution_source).toBe("ai");
  expect(account?.attribution_answered_at).not.toBeNull();
