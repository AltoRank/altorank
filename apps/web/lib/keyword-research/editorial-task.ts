...
import { supportsArticleTask, validResultRelevance, type QualificationAssessment, type ResultRelevance } from "./qualification-decision";
...
export async function preserveEditorialTask(query: string, organic: SerpData["organic"], assessment: QualificationAssessment, spend?: SpendSink, businessEvidence?: string, focus?: BusinessFocus | null): Promise<ReviewedEditorialTask | null> {
...
export interface ReviewedEditorialTask { queryTask:string; sourceQuote:string; angle:string; buyingJob:string; reason:string; supportingResultIndices:number[] }
...
export async function checkEditorialTask(query: string, organic: SerpData["organic"], assessment: QualificationAssessment, spend?: SpendSink, businessEvidence?: string, tier:ModelTier="editorial", focus?: BusinessFocus | null): Promise<EditorialTaskCheck> {
  ...
  const raw = await askStructured("keyword-research/editorial-task", [
    ...
    'For a supported task return JSON {"status":"supported","queryTask":string,"supportingResults":[{"resultIndex":number,"relevance":{"buyer":true,"task":true,"reason":string}}],"angle":string,"buyingJob":string,"reason":string,"businessFit":{"supported":true,"quote":string}}. businessFit.quote must be an exact substring of BUSINESS EVIDENCE establishing the actual core offering. The server attaches supporting result titles. Angle at most 140 characters. Reason explains how this headline answers the search. When the task or offering is unsupported return ONLY {"status":"unsupported","reason":string} explaining why, with no null-filled task object. Return only JSON, without analysis.',
    ...
    JSON.stringify({query,results:organic.map(({title,description}, resultIndex) => ({title,description,resultIndex})),eligibleArticleIndices:[...eligibleIndices],articleEvidence:eligible.map(({resultIndex,quote,relevance}) => ({resultIndex,quote,relevance})),proposed:{angle:assessment.angle,buyingJob:assessment.buyingJob,audience:assessment.audience}}),
  ].join("\n"), {maxTokens:1500,spend,tier});
  if (raw?.trim() === "null") return {status:"unsupported"};
  const checked = extractJson<{focusFit?:{buyer:boolean;offering:boolean;reason:string};status?:string;businessFit?:{supported:boolean;quote:string};queryTask:string;supportingResults?:Array<{resultIndex:number;relevance:ResultRelevance}>;angle:string;buyingJob:string;reason:string}>(raw,"{","}");
  ...
  return {status:"supported",task:{queryTask:checked.queryTask,sourceQuote:organic[supportingResultIndices[0]].title,angle:checked.angle,buyingJob:checked.buyingJob,reason:checked.reason.slice(0,400),supportingResultIndices}};
}
