/**
 * The AltoRank MCP server, transport-agnostic.
 *
 * Two entry points build it:
 *   scripts/mcp.ts        stdio, for a local agent (`npm run mcp`)
 *   app/api/mcp/route.ts  Streamable HTTP, the hosted endpoint any MCP client
 *                         adds as a connector (ChatGPT, Claude.ai, Cursor...)
 *
 ...
 *   account    the agent API (/api/agent/v1) through the `call` the entry
 *              point supplies: the stdio server sends ALTORANK_API_KEY over
 *              HTTP, the hosted route forwards the bearer token it was given.
 *              Same envelope, same client the CLI uses.
 *
 ...
 * skills/altorank/SKILL.md is the agent-facing statement of the same rules.
 *
 * Relative imports on purpose: tsx runs scripts/mcp.ts outside Next, without
 * the "@/" alias.
 */

import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { z } from "zod";
import { runAgentReadiness } from "../audit/agent-readiness";
import { proposeSchema, renderJsonLd } from "../audit/schema-generator";
import { htmlToMarkdown, buildLlmsTxt } from "../audit/markdown";
import { buildReadinessReport } from "../audit/readiness-report";
import { fail, ok, type AgentErrorCode, type Envelope } from "../agent/envelope";

/** One call against /api/agent/v1. Returns an envelope even on transport failure. */
export type AgentCall = <T = unknown>(
  path: string,
  opts?: { method?: "GET" | "POST"; body?: unknown; query?: Record<string, string | number | undefined | null> },
) => Promise<Envelope<T>>;

export type ServerOptions = {
  /** Replaces the last sentence of the server instructions: how the account tools get their credential. */
  accountNote?: string;
};
...
export function createAltorankServer(call: AgentCall, options: ServerOptions = {}): McpServer {
  ...
  { name: "altorank", version: "0.3.0" },
  {
    instructions:
      ...
      "Read skills/altorank/SKILL.md before using the account tools: preflight with altorank_whoami, pick a " +
      ...
      (options.accountNote ?? "Account tools need ALTORANK_API_KEY (create one at /settings/api-keys)."),
  },
);
...
server.registerTool(
  ...
  async () => asEnvelope(await call("/auth/whoami")),
);
...
server.registerTool(
  ...
  async () => asEnvelope(await call("/workspaces")),
);
...
server.registerTool(
  ...
  async ({ workspace_id }) => asEnvelope(await call(`/workspaces/${workspace_id}`)),
);
...
server.registerTool(
  ...
  async ({ workspace_id, status, limit }) =>
    asEnvelope(await call("/keywords", { query: { workspace_id, status, limit } })),
);
...
server.registerTool(
  ...
  async ({ workspace_id, seeds, limit }) =>
    asEnvelope(await call("/keywords/suggest", { method: "POST", body: { workspace_id, seeds, limit } })),
);
...
server.registerTool(
  ...
  async ({ workspace_id, status, limit }) =>
    asEnvelope(await call("/articles", { query: { workspace_id, status, limit } })),
);
...
server.registerTool(
  ...
  async ({ article_id }) => asEnvelope(await call(`/articles/${article_id}`)),
);
...
server.registerTool(
  ...
  async ({ article_id, format }) =>
    asEnvelope(await call(`/articles/${article_id}/content`, { query: { format } })),
);
...
server.registerTool(
  ...
  async (input) => asEnvelope(await call("/articles/generate", { method: "POST", body: input })),
);
...
server.registerTool(
  ...
  async () => asEnvelope(await call("/usage")),
);
...
server.registerTool(
  ...
  async ({ workspace_id, days }) => asEnvelope(await call("/gsc/performance", { query: { workspace_id, days } })),
);
...
server.registerTool(
  ...
  async ({ workspace_id, days, min_impressions, limit }) =>
    asEnvelope(await call("/gsc/cannibalization", { query: { workspace_id, days, min_impressions, limit } })),
);
...
server.registerTool(
  ...
  async ({ workspace_id, days, bucket }) => asEnvelope(await call("/gsc/coverage", { query: { workspace_id, days, bucket } })),
);
...
server.registerTool(
  ...
  async ({ workspace_id, url, days }) => asEnvelope(await call("/gsc/url-inspection", { query: { workspace_id, url, days } })),
);
...
server.registerTool(
  ...
  async ({ workspace_id, status }) => asEnvelope(await call("/keywords/export", { query: { workspace_id, status, format: "json" } })),
);
...
server.registerTool(
  ...
  async (input) => asEnvelope(await call("/keywords/bulk-reschedule", { method: "POST", body: input })),
);
...
server.registerTool(
  ...
  async (input) => asEnvelope(await call("/keywords/bulk-remove", { method: "POST", body: input })),
);
...
server.registerTool(
  ...
  async ({ article_id, ...body }) => asEnvelope(await call(`/articles/${article_id}/replace`, { method: "POST", body })),
);
...
server.registerTool(
  ...
  async (input) => asEnvelope(await call("/articles/bulk-replace", { method: "POST", body: input })),
);
...
server.registerTool(
  ...
  async ({ article_id }) => asEnvelope(await call(`/articles/${article_id}/retry-publish`, { method: "POST" })),
);
...
server.registerTool(
  ...
  async ({ workspace_id }) => asEnvelope(await call(`/workspaces/${workspace_id}/pause`, { method: "POST" })),
);
...
server.registerTool(
  ...
  async ({ workspace_id }) => asEnvelope(await call(`/workspaces/${workspace_id}/resume`, { method: "POST" })),
);
