...
/**
 * One delivery attempt, reported by adapters that retry over HTTP (the webhook
 * and the WordPress plugin). The publish core turns each one into a
 * publish_log row, so an endpoint that failed twice and then took the article
 * shows all three tries.
 */
