...
/** The wp/v2 `status` a payload asks for: the connection's publish mode, live when unset. */
function statusFor(article: PublishPayload): "publish" | "draft" {
  return article.publishMode === "draft" ? "draft" : "publish";
}
...
export class WordPressAdapter implements CMSAdapter {
  ...
  /** Edit the post in place. Same body as publish, so the connection's mode applies to the edit too. */
