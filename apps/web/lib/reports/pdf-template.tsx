...
export function ReportPDF({ data }: ReportPDFProps) {
  const accentColor = data.account.accent_color ?? "#2563eb";
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          <View>
            {data.account.logo_url && (
              <Image src={data.account.logo_url} style={styles.logo} />
            )}
          ...
          <View>
            <Text style={{ fontSize: 11, fontWeight: "bold" }}>{data.account.name}</Text>
          </View>
        </View>
        ...
        <Text style={styles.footer}>
          Generated by {data.account.name}{data.account.remove_branding ? "" : " — Powered by AltoRank"}
        </Text>
      </Page>
    </Document>
  );
