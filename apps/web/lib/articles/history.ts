...
export interface HistoryRow {
  ...
  canPublish: boolean;
  /** The last publish failed, so the row menu offers "Retry publish" instead of starting over (#83). */
  canRetry: boolean;
  /** Search Console clicks over the last 30 days, page rows only. Null when nothing was attributed (#84). */
  clicks: number | null;
  /** Index coverage for a published URL: inspection first, else "served in search". Null when not published (#84). */
  index: { bucket: CoverageBucket; title: string } | null;
}

/** The optional extras the page computes from other reads; every one has an honest default. */
export type HistoryRowExtras = Partial<Pick<HistoryRow, "canRetry" | "clicks" | "index">>;
...
export function toHistoryRow(a: Article, canPublish: boolean, extras: HistoryRowExtras = {}): HistoryRow {
  return {
