...
export async function checkEvidenceScope(brief: Record<string,string>, questions: string[], spend?: SpendSink): Promise<EvidenceScope> {
  // Qualification explanations and the wider product catalog justify relevance;
  // they are not additional promises made by this particular article.
  const contract=Object.fromEntries(["angle","buyingJob","audience","instructions"].flatMap(key=>typeof brief[key]==="string"?[[key,brief[key]]]:[]));
  const raw = await askStructured("article/evidence-scope",[
    ...
    "The approved headline defines the bounded article promise. When it explicitly names comparison criteria, judge completeness against those criteria and any explicit additional instruction; do not require every ordinary feature of the product category. The buying job explains the reader's purpose, not a mandate to add all possible ways of achieving it. A general goal such as coordinating projects or choosing an email tool does not add an unnamed feature criterion to a comparison with an explicit scope. Explicit instructions can add a required criterion or action and must be respected.",
    "Research questions establish factual inputs. Weighing the supported criteria, applying them to the reader's situation, and synthesizing a recommendation belong to writing; they do not each require an additional source question. Do not mark coverage incomplete merely because there is no separate question asking for an overall verdict. A promised cost comparison still needs comparable cost facts, and a promised procedure still needs its actual actions, prerequisites and material limits.",
    ...
    JSON.stringify({brief:contract,questions:questions.map((question,requirementIndex)=>({requirementIndex,question}))}),
