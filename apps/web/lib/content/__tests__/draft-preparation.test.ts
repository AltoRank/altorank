...
import {draftPreparationContext,loadDraftPreparation,prepareDraft,readDraftPreparation,type DraftPreparation,type DraftPreparationInput} from "../draft-preparation";
...
const plan={status:"planned" as const,task:"explanation" as const,requirements:["How do reviews work?"],selectedUrls:[],retrievedUrls:[sources[0].url]};
const sourceBrief={status:"prepared" as const,facts:[{subject:"Example",plan:"",kind:"capability" as const,statement:sources[0].text,sourceIndex:0,quote:sources[0].text,scopeQuote:sources[0].text,url:sources[0].url}],coverage:[{question:plan.requirements[0],factIndices:[0]}],issues:[],readiness:{status:"checked" as const,questions:[{requirementIndex:0,answered:true,reason:"Direct instructions."}]}};
function packet():DraftPreparation{return structuredClone({version:1,context:draftPreparationContext(input),createdAt:new Date(Date.now()-1000).toISOString(),expiresAt:new Date(Date.now()+3600000).toISOString(),status:"ready",sources,plan,sourceBrief});}
...
it.each(["workspaceId","keywordId","keyword","domain","language","locationCode","profile","instructions","brief"] as const)("invalidates preparation when %s changes",async(field)=>{
...
it.each(["expired","future","missing-questions","duplicate-indices","false-answer","empty-facts","wrong-coverage"])("rejects %s cache records without throwing",kind=>{
  const p=packet();
  ...
  expect(readDraftPreparation(p,p.context)).toBeNull();
});
