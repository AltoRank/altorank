...
drop policy if exists "Refresh candidates by access" on refresh_candidates;
...
drop policy if exists "Refresh tasks by access" on refresh_tasks;
...
drop policy if exists "Refresh executions by access" on refresh_executions;
...
drop policy if exists "Research runs by access" on keyword_research_runs;
...
drop policy if exists "Link sources by access" on link_sources;
create policy "Link sources by access" on link_sources
  for all using (workspace_id in (select user_workspace_ids()))
  with check (workspace_id in (select user_workspace_ids()));

drop policy if exists "Link targets by agency" on link_targets;
drop policy if exists "Link targets by access" on link_targets;
