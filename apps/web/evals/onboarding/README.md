...
- `claims.json`: 19 synthetic passage cases, derived from failure classes seen in four live businesses. Eleven development cases and eight initially reserved cases, with correct/incorrect minimal pairs. Includes Italian, plan limits, conditional advice, attribution, repetition and unsupported inferences.
...
The first-choice planner also groups synonymous buyer tasks after qualification, preserving the highest-ranked representative of each group. One extra call has a 15-second ceiling. A malformed or unavailable partition keeps one already-qualified option rather than filling the calendar with unchecked variations; stored keyword eligibility is unchanged. This applies only to explicit first-draft choice, not normal calendar top-ups.

Replay observed topic overlap with `onboarding-diversity-eval.ts --provider-env=... --report=... --out=...`. Inspect both the complete partition and kept indices: the one-topic fallback is not a passing model judgment. The Cal.com three-synonym set and its distinct payment/group-availability control each ran twice. Added `flow-*` passage cases test incorrect any-host/all-host failure criteria, the corrected test, and an actual linked cross-reference. Use `onboarding-eval.ts --case-prefix=flow- --variants=combined --split=development` to isolate them.


The Tally follow-up adds one optional evidence-planning round through newly observed vendor links, at most three further reads and twelve retrieved pages overall. Pricing references found beyond large product menus are retained within the 80-link output cap. A truncated claim batch may split once into smaller assignments within the original eight-call, three-concurrent, 90-second limits; failed retries stay partial. The writer no longer treats unsourced qualitative generalizations as publishable substitutes for missing statistics. Source availability must not be presented as a product advantage.
