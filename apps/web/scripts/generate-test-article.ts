...
async function main(): Promise<void> {
  ...
  // Ask the resolver itself rather than counting articles here. This printed
  // "0" on a site with 204 crawled pages, because it only looked at the
  // `articles` table while fetchLinkTargets reads site_pages too - so the run
  // that finally produced ten internal links reported having nothing to link
  // to. A diagnostic that disagrees with the thing it is diagnosing is worse
  // than no diagnostic.
  const targets = await fetchLinkTargets(db, workspaceId!);
  const liveCount = targets.length;
  console.log(`  link targets available: ${liveCount}`);
